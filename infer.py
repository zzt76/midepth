...
import model_io
import utils
from zoo.unet_v1_1 import UNet
...
class InferenceHelper:
    def __init__(self, dataset='nyu', device='cuda:0', pretrained_path=None):
        ...
        if dataset == 'nyu':
            ...
            model = UNet(cout=1)
            if pretrained_path == None:
                pretrained_path = "checkpoints_kitti/WeightedUnet_29.pt"
        elif dataset == 'mi':
            self.min_depth = 1e-3
            self.max_depth = 10
            self.saving_factor = 1  # used to save in 16 bit
            model = UNet(cout=1)
            if pretrained_path == None:
                pretrained_path = "checkpoints_ml/WeightedUnet_29.pt"
        else:
        ...
        model, _, _, _ = model_io.load_checkpoint(pretrained_path, model)
        model.eval()
    ...
    def predict_pil(self, pil_image, visualized=False):
        ...
        # img = tf.to_tensor(pil_image).unsqueeze(0).float().to(self.device)
    ...
    def predict(self, image):
        ...
        final = 0.5 * (pred + pred_lr) * self.max_depth
